...
/// A special launcher that knows how to bootstrap the
/// Malachite consensus client.
///
/// Largely based on `DefaultNodeLauncher` from reth v1.0.4, with
/// modifications to swap `AutoSealBuilder` for `MalachiteBuilder`.
#[derive(Debug)]
        ...
        EthApiBuilderProvider<NodeAdapter<T, CB::Components>> + FullEthApiServer + AddDevSigners,
{
    ...
    async fn launch_node(
        ...
        let (pipeline, client) = if ctx.is_dev() {
            ...
            // adi: this is not needed anymore
            // todo cleanup
            // let mining_mode =
